...
def denormalize_station_prices_for_dataviz(year_to_load = None, drop_mongo_collections = None):
    ...
    if drop_mongo_collections == "true":
        ...
        mongo_manager.drop_mongo_collections(bdd = "denormalization", collections= ["denormalized_station_prices"])
    start_date_to_load, end_date_to_load = utils.determine_dates_to_load_from_mongo(year_to_load, db_name= "denormalization", collection= "denormalized_station_prices")
...
def extract_new_station_prices_from_mongo(start_date_to_load, end_date_to_load):
    ...
    if os.path.exists("outputs/denormalized_station_prices"):
        shutil.rmtree("outputs/denormalized_station_prices")
    os.makedirs("outputs/denormalized_station_prices", exist_ok=True)
...
def load_denormalized_station_prices(df_denorm_station_prices):
    print("[INFO] Start load_denormalized_station_prices")

    # Save df to csv
    start_year = df_denorm_station_prices['Date'].min().year
    end_year = df_denorm_station_prices['Date'].max().year
    df_denorm_station_prices.to_csv(f"outputs/denormalized_station_prices/denorm_station_prices_{start_year}_{end_year}.csv", index=False)

    # Save df to Mongo
    result = mongo_manager.load_datas_to_mongo(df_denorm_station_prices, bdd="denormalization",collection="denormalized_station_prices", index=["Date"])
    if result:
        print(f"correctly loaded denormalized_station_prices_{start_year}_{end_year} on mongo collection 'denormalization'")

    print(f"END LOAD denormalized_station_prices_{start_year}_{end_year}")
    return "done"
